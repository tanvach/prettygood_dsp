...
def main():
    ...
    # Begin changes
    eq_kwargs = {
        'equalize': True, 'parametric_eq': True, 'max_filters': [7], 'ten_band_eq': False, 'new_only': new_only,
        'convolution_eq': False, 'fs': [44100]
    }
    onear_kwargs = eq_kwargs.copy()
    onear_kwargs.update({'bass_boost_gain': 0.0})
    inear_kwargs = eq_kwargs.copy()
    inear_kwargs.update({'bass_boost_gain': 0.0})
